...
BEGIN
  ...
  ELSE
    ...
    LOOP
      ...
      IF v_task_tier_id IS NULL THEN CONTINUE; END IF;

      DECLARE sub_r record;
      BEGIN
        ...
        LOOP
          IF sub_r.commission_type IS NOT NULL AND sub_r.commission_value > 0 THEN
            ...
            IF v_assignee_count > 0 THEN
              DECLARE v_per_assignee numeric(12,2); sa record;
              BEGIN
                ...
                LOOP
                  IF NOT EXISTS (SELECT 1 FROM public.task_commissions tc WHERE tc.task_id = r.task_id AND tc.subtask_id = sub_r.subtask_id AND tc.user_id = sa.user_id AND tc.manual_override = true) THEN
                    ...
                    ON CONFLICT (task_id, user_id) DO UPDATE SET amount = EXCLUDED.amount, status = 'confirmed', commission_source = 'subtask', subtask_id = sub_r.subtask_id, manual_override = false, updated_at = now();
          ...
          ELSE
            IF sub_r.sub_mode = 'role' THEN
              ...
              BEGIN
                ...
                LOOP
                  v_rate := 0;
                  SELECT rr.rate INTO v_rate FROM public.commission_rate_card rc
                  JOIN public.rate_card_rates rr ON rr.rate_card_id = rc.id AND rr.tier_id = v_task_tier_id
                  WHERE rc.organization_id = v_org_id AND rc.category = 'role'
                    AND lower(rc.name) = lower(sa_role.role) AND rc.complexity IS NULL;
                  v_rate := COALESCE(v_rate, 0) * sub_r.sub_quantity;
                  IF v_rate > 0 AND NOT EXISTS (SELECT 1 FROM public.task_commissions tc WHERE tc.task_id = r.task_id AND tc.subtask_id = sub_r.subtask_id AND tc.user_id = sa_role.user_id AND tc.manual_override = true) THEN
                    INSERT INTO public.task_commissions (project_id, task_id, subtask_id, user_id, amount, status, commission_source, manual_override, updated_at)
                    VALUES (p_project_id, r.task_id, sub_r.subtask_id, sa_role.user_id, v_rate, 'confirmed', 'task_manager', false, now())
                    ON CONFLICT (task_id, user_id) DO UPDATE SET amount = EXCLUDED.amount, status = 'confirmed', commission_source = 'task_manager', subtask_id = sub_r.subtask_id, manual_override = false, updated_at = now();
            ...
            ELSIF sub_r.sub_mode = 'type' AND sub_r.sub_work_type IS NOT NULL THEN
              ...
              BEGIN
                v_type_rate := 0;
                SELECT rr.rate INTO v_type_rate FROM public.commission_rate_card rc
                JOIN public.rate_card_rates rr ON rr.rate_card_id = rc.id AND rr.tier_id = v_task_tier_id
                WHERE rc.organization_id = v_org_id AND rc.category = 'deliverable'
                  AND lower(rc.name) = lower(sub_r.sub_work_type)
                  AND (lower(coalesce(rc.complexity,'')) = lower(coalesce(sub_r.sub_complexity,'')));
                ...
                IF v_type_rate > 0 THEN
                  ...
                  LOOP
                    IF NOT EXISTS (SELECT 1 FROM public.task_commissions tc WHERE tc.task_id = r.task_id AND tc.subtask_id = sub_r.subtask_id AND tc.user_id = sa_type.user_id AND tc.manual_override = true) THEN
                      INSERT INTO public.task_commissions (project_id, task_id, subtask_id, user_id, amount, status, commission_source, manual_override, updated_at)
          ...
          IF sub_r.commission_type IS NOT NULL AND sub_r.commission_value > 0 THEN
            ...
            IF v_assignee_count > 0 THEN
              ...
              BEGIN
                ...
                LOOP
                  UPDATE public.user_wallets SET potential_balance = potential_balance + v_per, updated_at = now() WHERE user_id = sa2.user_id;
                END LOOP;
              END;
            END IF;
          ELSE
            IF sub_r.sub_mode = 'role' THEN
              ...
              BEGIN
                ...
                LOOP
                  ...
                  v_rate := COALESCE(v_rate, 0) * sub_r.sub_quantity;
                  IF v_rate > 0 THEN
                    UPDATE public.user_wallets SET potential_balance = potential_balance + v_rate, updated_at = now() WHERE user_id = sa_role.user_id;
                  END IF;
                END LOOP;
              END;
            ELSIF sub_r.sub_mode = 'type' AND sub_r.sub_work_type IS NOT NULL THEN
              ...
              BEGIN
                ...
                IF v_type_rate > 0 THEN
                  FOR sa_type IN SELECT user_id FROM public.subtask_assignees WHERE subtask_id = sub_r.subtask_id
                  LOOP
                    UPDATE public.user_wallets SET potential_balance = potential_balance + v_type_rate, updated_at = now() WHERE user_id = sa_type.user_id;
                  END LOOP;
                END IF;
              END;
            END IF;
